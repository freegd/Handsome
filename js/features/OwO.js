    ...
    var OwO = function () {
        ...
        _createClass(OwO, [{
                ...
                for (var i = 0; i < this.packages.length; i++) {
                    ...
                    for (var _i = 0; _i < opackage.length; _i++) {
                        if (this.odata[this.packages[i]].type === 'image'){
                            html += '\n                    <li class="OwO-item" title="' + opackage[_i].text + '" data-input="' + this.odata[this.packages[i]].name + ":" + opackage[_i].icon +'">' + '<img data-src="' + LocalConst.BASE_SCRIPT_URL + 'usr/img/emotion/'+ this.odata[this.packages[i]].name + '/' + opackage[_i].icon + '.png" icon="' + opackage[_i].text +'"></li>';
                        }else{
            ...
            value: function toggle() {
                ...
                //懒加载该区域类的图片
                if (this.container.classList.contains("OwO-open")) {
                    var a = this.container.querySelectorAll(".OwO-items.OwO-items-show>li img"); [].forEach.call(a,
                        function(a) {
                            var b = a.dataset.src;
